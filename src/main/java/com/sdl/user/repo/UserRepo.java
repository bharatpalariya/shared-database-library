...
import com.sdl.user.common.UserVariablesConstant;
...
public interface UserRepo extends JpaRepository<User, Long> {
    ...
    @Query(value = "select count(*) from users u where u.username = :username and u.status = :status", 
           nativeQuery = true)
    Integer fetchCountByUsernameAndStatus(@Param(UserVariablesConstant.USERNAME) String username, 
                                        @Param(UserVariablesConstant.STATUS) String status);
